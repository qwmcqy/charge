...
export class User {
  ...
  async getChargingProgress(orderId: string) {
    ...
    const energyDiff = this.batteryCapacity * (order.target_battery_level - order.request_battery_level) / 100;
    const progress = order.status === 'charging'
      ? Math.min(100, order.request_battery_level + (order.energy_consumed / this.batteryCapacity) * 100)
      : order.request_battery_level;
    const estimatedRemaining = station
      ? (order.target_battery_level - progress) / (station.mode === 'fast' ? 2.0 : 0.5)
      : 0;
