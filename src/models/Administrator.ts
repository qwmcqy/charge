...
export class Administrator {
  ...
  async generateBillForOrder(orderId: string): Promise<BillData> {
    ...
    const { calculateTimeOfUseFee } = await import('@/lib/billing');
    const chargingFee = order.start_time
      ? calculateTimeOfUseFee(new Date(order.start_time), order.energy_consumed, order.mode).totalFee
      : Math.round((order.charging_fee || 0) * 100) / 100;
