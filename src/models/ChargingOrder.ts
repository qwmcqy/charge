...
export class ChargingOrder {
  ...
  async endCharging(endStatus: OrderStatus = OrderStatus.Completed): Promise<void> {
    ...
    await supabase
      .from('charging_orders')
      .update({ status: endStatus, end_time: this.endTime.toISOString(), charging_fee: this.chargingFee })
      .eq('id', this.id);
  ...
  calculateChargingFee(ratePerKwh: number): number {
    this.chargingFee = this.startTime
      ? calculateTimeOfUseFee(this.startTime, this.energyConsumed, this.mode).totalFee
      : Math.round(this.energyConsumed * ratePerKwh * 100) / 100;
