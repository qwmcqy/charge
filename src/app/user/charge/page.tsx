...
export default function ChargePage() {
  ...
  const [requestedKwh, setRequestedKwh] = useState(30);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/charging/request', {
        ...
        body: JSON.stringify({ userId, mode, batteryLevel, requestedKwh }),
      });
      ...
      <div>
        ...
        <div className="bg-white rounded-xl shadow p-8 text-center">
          ...
          <div className="grid grid-cols-2 gap-4 mt-6 text-left">
            ...
            <div className="p-4 bg-gray-50 rounded-lg">
              <p className="text-sm text-gray-500">请求充电量</p>
              <p className="text-lg font-bold">{result.requestedKwh} kWh</p>
            </div>
            <div className="p-4 bg-gray-50 rounded-lg">
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow p-6 space-y-6 max-w-lg">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">请求充电量: {requestedKwh} kWh</label>
          <input type="range" min="1" max="100" value={requestedKwh} onChange={e => setRequestedKwh(Number(e.target.value))}
            className="w-full h-2 bg-gray-200 rounded-full appearance-none cursor-pointer accent-green-600" />
          <div className="flex justify-between text-xs text-gray-400 mt-1"><span>1</span><span>50</span><span>100 kWh</span></div>
        </div>
